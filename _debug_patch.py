...
exec((ROOT / "_patch_all.py").read_text(encoding="utf-8").split("def patch_tests")[0])
...
spec = importlib.util.spec_from_file_location("pa", ROOT / "_patch_all.py")
pa = importlib.util.module_from_spec(spec)
# patch patch_generate to add debug
source = (ROOT / "_patch_all.py").read_text(encoding="utf-8")
...
exec(compile(source, str(ROOT / "_patch_all.py"), "exec"), ns)
ns["patch_generate"]()

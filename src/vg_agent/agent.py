...
def _run_live_subagent(
    ...
    # Reviewer is bounded tighter than other sub-agents: a verdict on one
    # changed file needs <=2 tool calls, so cap its loop to contain runaway
    # token cost. On exhaustion the deterministic FAIL fallback still fires.
    step_cap = config.MAX_REVIEWER_STEPS if agent_type == "reviewer" else config.MAX_SUBAGENT_STEPS

    for local_step in range(1, step_cap + 1):
